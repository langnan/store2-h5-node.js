$(function () {
  ...
  function drawImageOfCanvasSize() {
    ...
    ctx.fillText(name, canvas.width * 0.28 - (name.length - 1) * 7 * scale, canvas.height * 0.46);
    ctx.fillText(number, canvas.width * 0.228 - (number.length - 1) * 4 * scale, canvas.height * 0.52);
    var myDate = new Date();
    ...
    ctx.fillText(time, canvas.width * 0.666, canvas.height * 0.8);
    ctx.stroke();
